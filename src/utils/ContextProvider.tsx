...
export const CollectionProvider: FC<{ children: ReactNode }> = ({
  children,
}) => {
  const [collection, setCollection] = useState<
    { amount: number; name: string; imgSrc: string }[]
  >([]);
  ...
  const clearAmount = (id: string, imgSrc: string) => {
    ...
    setCollection((prevCollection) =>
      prevCollection.filter(
        (item) => !(item.name === id && item.imgSrc === imgSrc)
      )
    );
  };
  ...
  return (
    <CollectionContext.Provider
      value={{ collection, increment, decrease, clearAmount }}
    >
      {children}
      <form className="flex flex-col justify-center items-center gap-4">
        <span className="w-full text-center">
          {collection.length > 0 ? "your list contains of:" : null}
        </span>
        {collection.length > 0
          ? collection.map((e, index) => (
              <div
                key={index}
                className="flex justify-center items-center gap-4 w-full border-b-2 pb-4"
              >
        ...
        />
        <button
          className="rounded-md border-4 border-green-500 text-white p-2 col-span-full mb-5"
          type="submit"
        >
          Add to collection
        </button>
