...
export const Login = async (formData: FormData) => {
  ...
  if (user) {
    ...
    if (passwordMatches) {
      ...
      (
        await // step 3: userId in cookie speichern
        cookies()
      ).set("user", user.id as string, { secure: true });
      redirect("/collection");
    }
  }
};
...
export const signUp = async (formData: FormData) => {
  ...
  if (user) {
    ...
    if (passwordMatches) {
      (await cookies()).set("user", user.id as string, { secure: true });
      redirect("/collection");
    }
  }
};
...
export const changeNickname = async (formData: FormData) => {
  const userId = (await cookies()).get("user");
...
export const deleteUser = async (formData: FormData) => {
  ...
  if (user) {
    ...
    if (passwordMatches) {
      // step 3: user löschen
      // eslint-disable-next-line @typescript-eslint/no-unused-vars
      const deleteUser = sql`DELETE FROM users WHERE email = ${
        email as string
      }`;
    }
    redirect("/");
...
export const createCollection = async (collection: Collection[]) => {
  const userId = (await cookies()).get("user");
  const collectionJSOn = collection.map((e) => ({
    amount: e.amount,
    name: e.name,
    imgSrc: e.imgSrc,
  }));
  ...
  const addCollection =
    await sql`INSERT INTO collections (id, name, cards) VALUES (
  ${userId!.value},  
  'test',
    '[
